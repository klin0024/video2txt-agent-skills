...
SUPPORTED_OUTPUT_SUFFIXES = {".srt", ".txt", ".ass"}
...
def build_output_paths(
    ...
    if output_path is not None:
        ...
        return target_parent / f"{base_name}.srt", target_parent / f"{base_name}.txt", target_parent / f"{base_name}.ass"

    target_dir = output_dir if output_dir is not None else input_path.parent
    target_dir.mkdir(parents=True, exist_ok=True)
    stem = input_path.stem
    return target_dir / f"{stem}.srt", target_dir / f"{stem}.txt", target_dir / f"{stem}.ass"
...
def render_ass(segments: list[TranscriptSegment]) -> str:
    header = (
        ...
        "Style: Default,Arial,60,&H00000000,&H000000FF,&H00FFFFFF,&H80000000,"
...
def main() -> int:
    ...
    try:
        ...
        srt_path, txt_path, ass_path = build_output_paths(input_path, output_dir, output_path)
        log(f"Planned SRT output: {srt_path}")
        log(f"Planned TXT output: {txt_path}")
        log(f"Planned ASS output: {ass_path}")
    ...
    ass_path.write_text(render_ass(segments), encoding="utf-8-sig")
